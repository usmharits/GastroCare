...
<head>
    ...
    <title>Credential Update - Sistem Pakar Diagnosa Penyakit Lambung</title>
        ...
        <div class="row m-0 h-full">
            ...
            <div class="col-lg-6 col-12 h-full flex items-center justify-center p-4 sm:p-8 bg-bgsoft relative overflow-hidden">
                ...
                <div class="w-full max-w-[420px] bg-cardbg rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-borderline p-6 sm:p-8 transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)]">
                    ...
                    <div class="w-12 h-12 bg-bgsoft rounded-2xl flex items-center justify-center border border-borderline mb-5 mx-auto lg:mx-0">
                        <svg class="w-6 h-6 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" /></svg>
                    </div>

                    <div class="mb-6 text-center lg:text-left">
                        <h2 class="text-xl font-bold text-textmain mb-1.5 tracking-tight">Credential Update</h2>
                        <p class="text-[13px] text-textmain/60 leading-relaxed">Silakan tentukan kata sandi baru untuk memulihkan akses ke akun Anda.</p>
                    </div>

                    <form method="POST" action="{{ route('password.store') }}" id="resetForm" class="space-y-4">
                        @csrf
                        
                        <input type="hidden" name="token" value="{{ $request->route('token') }}">

                        <div class="group">
                            <label for="email" class="block text-[12px] font-semibold text-textmain mb-1.5">Validation Email</label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                                    <svg class="h-4 w-4 text-textmain/40" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m4.5-1.206a8.959 8.959 0 01-4.5 1.207" /></svg>
                                </div>
                                <input type="email" id="email" name="email" value="{{ old('email', $request->email) }}" required autofocus
                                    class="w-full py-2.5 pl-9 pr-4 bg-[#F8FAFC] border border-borderline rounded-xl text-[13px] text-textmain/70 focus:outline-none cursor-not-allowed select-none"
                                    readonly tabindex="-1">
                            </div>
                            <p class="text-[11px] font-medium text-status-info mt-1.5 flex items-center gap-1">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                Email ini terikat dengan sesi pemulihan Anda.
                            </p>
                        </div>

                        <div class="grid grid-cols-2 gap-3">
                            <div class="group relative">
                                <label for="password" class="block text-[12px] font-semibold text-textmain mb-1.5">New Password</label>
                                <div class="relative">
                        ...
                        <div class="pt-3">
                            <button type="submit" id="submitBtn" class="w-full py-3 bg-primary hover:bg-[#233B60] text-white rounded-xl font-semibold text-[13px] transition-all duration-200 shadow-[0_4px_14px_rgba(47,79,127,0.3)] hover:shadow-[0_6px_20px_rgba(47,79,127,0.4)] flex justify-center items-center gap-2 group active:scale-[0.98]">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            @if($errors->any())
                Swal.fire({
                    icon: 'error',
                    title: 'Update Failed',
                    text: '{{ $errors->first() }}', // Menangkap pesan error spesifik (misal: sandi tidak cocok)
            ...
            form.addEventListener('submit', function() {
                ...
                Swal.fire({
                    ...
                    html: '<span class="text-sm text-gray-500">System is saving your new password...</span>',
