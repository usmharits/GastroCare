...
<head>
    ...
    <title>System Login - Sistem Pakar Diagnosa Penyakit Lambung</title>
    ...
    <style>
        ...
        /* Disable scroll section completely for desktop to ensure perfect fit */
    ...
    <div class="container-fluid p-0 h-full">
        <div class="row m-0 h-full">
            ...
            <div class="col-lg-6 col-12 h-full flex items-center justify-center p-4 sm:p-8 bg-bgsoft relative overflow-hidden">
                ...
                <div class="w-full max-w-[380px] bg-cardbg rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-borderline p-6 sm:p-8 transition-all duration-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)]">

                    @if (session('status'))
                        <div class="mb-5 bg-status-success/10 border border-status-success/20 text-status-success px-4 py-2.5 rounded-xl text-xs font-medium flex items-center gap-2.5">
                    ...
                    <div class="mb-6 text-center lg:text-left">
                        <h2 class="text-xl font-bold text-textmain mb-1.5 tracking-tight">System Authentication</h2>
                        <p class="text-[13px] text-textmain/60 leading-relaxed">Silakan masukkan email dan password Anda untuk akses dasbor.</p>
                    </div>

                    <form method="POST" action="{{ route('login') }}" id="authForm" class="space-y-4">
                        @csrf
                        ...
                        <div class="flex items-center justify-between pt-1">
                            <label for="remember_me" class="inline-flex items-center cursor-pointer group" title="Sistem akan mengingat sesi Anda selama 30 hari">
                                <input id="remember_me" type="checkbox" name="remember" 
                                    class="w-3.5 h-3.5 rounded border-borderline text-primary focus:ring-primary/30 transition-colors cursor-pointer accent-primary">
                                <span class="ml-2 text-[12px] font-medium text-textmain/70 group-hover:text-primary transition-colors">Remember me</span>
                            </label>
                        ...
                        @if (Route::has('register'))
                            <div class="text-center pt-4 mt-2 border-t border-borderline/60">
                                <p class="text-[12px] text-textmain/60 font-medium">
                                    Belum memiliki akses? 
                                    <a href="{{ route('register') }}" class="text-primary font-bold hover:underline hover:text-[#233B60] transition-colors">Register Account</a>
                                </p>
                            </div>
                        @endif
                    </form>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            @if($errors->any())
                Swal.fire({
                    ...
                    text: 'Kredensial yang Anda masukkan tidak valid. Please check your email and password.',
            ...
            form.addEventListener('submit', function() {
                ...
                Swal.fire({
                    ...
                    html: '<span class="text-sm text-gray-500">System is verifying your credentials...</span>',
